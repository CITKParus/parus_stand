...
Описание: по полученному штрих-коду посетителя возвращает сведения о нём (рег. номер контрагента в ПП Парус 8, его мнемокод, наименование), а так же описание текущего состояния остатков вендингового аппарата (текущие товарные запасы по местам хранения), при этом на сервере приложений устанавливается флаг занятости вендингового автомата.


### Отмена аутентификация пользователя по штрих-коду (CANCEL_AUTH)
Способ передачи параметров: *POST, GET*
...
*action* - обязательный, строка, принимает значение "CANCEL_AUTH"

*customerID* - обязательный, число, регистрационный номер контрагента посетителя стенда, полученный после "AUTH_BY_BARCODE"

Описание: Сбрасывает флаг занятости вендингового автомата на сервере приложений. При сбросе проверяется соответствие идентификатора текущего пользователя стенда переданному "customerID". Для принудительного аварийного сброса состояния передайте в "customerID" значение указанное в константе "SERVER_USER_RESET_EMERGENCY_CODE" файла конфигурации (config.js).


### Загрузка вендингового автомата товаром (LOAD)
Способ передачи параметров: *POST, GET*
...
*action* - обязательный, строка, принимает значение "LOAD"

*rack_line* - необязательный, число, номер загружаемого яруса стеллажа стенда (стенд, это склад с одним стеллажом, разбитым на ярусы, если не указан - будут загружены все ярусы, обязателен, если указан "rack_line_cell")

*rack_line_cell* - необязательный, число, номер загружаемого места хранения яруса стеллажа стенда (каждый ярус разбит на ячейки, если не указан, будут загружены все ячейки, принадлежащие указанному "rack_line")

Описание: Создаёт на сервере ПП Парус 8 складской документ типа "Приход из подразделения" для указанного места хранения. При создании документа так же указывается место хранения тованого запаса. Документ отрабатывается как факт и производится приходование на места хранения. Возвращает идентификатор сформированного документа.


### Откат последней загрузки вендингового автомата товаром (LOAD_ROLLBACK)
Способ передачи параметров: *POST, GET*
...
*action* - обязательный, строка, принимает значение "LOAD_ROLLBACK"

Описание: Снимает отработку и удаляет на сервере ПП Парус 8 последний (по хронологии) складской документ типа "Приход из подразделения", зарегистрированный для склада стенда. Возвращает идентификатор удалённого документа.
...
### Откат отгрузки товара из вендингового автомата (SHIPMENT_ROLLBACK)
Способ передачи параметров: *POST, GET*
...
*action* - обязательный, строка, принимает значение "SHIPMENT_ROLLBACK"

*documentID* - обязательный, число, идентификатор отгрузочного документа, полученный после выполнения "SHIPMENT"

Описание: Откатывает на сервере ПП Парус 8 складской документ типа "Расходная накладная на отпуск потребителям" с указанным идентификатором.
