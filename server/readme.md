...
### Проверка состояния отчета по позиции очереди уведомлений стенда (MSG_GET_PRINT_STATE)
Способ передачи параметров: *POST, GET*
...
*action* - обязательный, строка, принимает значение "MSG_GET_PRINT_STATE"

*rn* - обязательный, число, уникальный идентификатор сообщения (приходит при вызове "MSG_GET_LIST" в поле "NRN" для каждого из сообщений)

Описание: Проверяет состояние позиции очереди печати, указанной в теле сообщения, переданного в параметре "rn". Возвращает JSON-описание состояния позиции очереди (с указанием ошибки печати, если была и типом файла для выгрузки). Если отчет уже обработан сервером печати, то для позиции очереди уведомлений, переданной в на сервер в параметре "rn" будет установлен стату "PRINTED".
